...
test("weekly reflection is an inline Today card for the profile owner, not a new route", () => {
  assert.match(today, /<WeeklyReflectionCard[\s\S]*profileId=\{props\.activeUser\}/i);
  assert.match(today, /props\.activeCanEdit && \([\s\S]*<WeeklyReflectionCard/i);
